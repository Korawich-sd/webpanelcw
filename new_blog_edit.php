...
if (isset($_POST['del-img'])) {
    ...
    if ($delete_img) {
        echo "<meta http-equiv='refresh' content='0;url=new_blog_edit?news_id=$news_id'>";
    }
}
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img_cover['size'] > 0 && $img_cover['error'] == 0) {
            if (move_uploaded_file($img_cover['tmp_name'], $filePath1)) {
                $news = $conn->prepare("UPDATE news SET news_name = :news_name ,content = :content, 
        ...
        $news = $conn->prepare("UPDATE news SET news_name = :news_name ,content = :content WHERE news_id = :news_id");
        $news->bindParam(":news_name", $news_name);
        $news->bindParam(":content", $text_content);
        $news->bindParam(":news_id", $news_id);
        $news->execute();
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <button type="submit" name="edit-news" class="btn btn-edit">Save</button>
                        </div>
                        <div class="title">
                            ...
                            <div class="title-img">
                                ...
                                <div id="gallery-cover">
                                    <div class='box-edit-img-cover'>
                                        ...
                                        <img class='edit-img-cover' id='previewImg-cover' src="upload/upload_news/<?php echo $row_news['cover_img'] ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="product-name">
                                ...
                                <input type="text" value="<?php echo $row_news['news_name'] ?>" name="news_name" class="form-control">
                            </div>

                            <div class="content-text">
                                <h6>Content</h6>
                                <textarea name="text-content"><?php echo $row_news['content'] ?></textarea>
                                <script>
                                    tinymce.init({
                                        selector: 'textarea',
                                        height: "350",
